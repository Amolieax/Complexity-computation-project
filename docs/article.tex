...
(1,2)-задача коммивояжёра является частным случаем метрической задачи коммивояжёра, в которой все веса рёбер принимают значения только 1 или 2. Несмотря на кажущуюся простоту, эта задача остаётся \textbf{NP-трудной}, что было доказано Карпом ещё в 1972 году. Однако для этого частного случая удаётся получить лучшие гарантии приближения по сравнению с общей метрической TSP.

\subsection{Исторический контекст}
До работы Бермана и Карпинского лучшим известным приближённым алгоритмом для (1,2)-TSP был алгоритм Пападимитриу и Яннакакиса с коэффициентом \(\frac{7}{6} \approx 1.1667\). В 2006 году Берман и Карпинский представили алгоритм, улучшающий эту границу до \(\frac{8}{7} \approx 1.1429\), что остаётся наилучшим известным результатом на сегодняшний день.
...
\begin{enumerate}
    \item \textbf{Пути}: компоненты, содержащие хотя бы одну вершины степени 1
...
\begin{itemize}
    ...
    \item Списки смежности для каждой вершина
...
\subsection{Графики сравнительного анализа}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{performance_analysis.png}
    \caption{Зависимость времени выполнения от размера графа \(n\).}
    \label{fig:time_vs_n}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{edge_distribution.png}
    \caption{Доля рёбер веса 1 в найденном туре в зависимости от размера графа.}
    \label{fig:edges_weight1}
\end{figure}

\subsection{Вывод консоли (примеры тестов)}

\begin{verbatim}
Размер графа: 8
Время выполнения: 0.1217 секунд
Стоимость тура: 9
Длина тура: 9 вершин
Ребра веса 1: 7 (87.5%)
Ребра веса 2: 1
Оптимальная стоимость: 9
Коэффициент приближения: 1.0000
Тур: [0, 1, 3, 2, 7, 5, 4, 6, 0]

Размер графа: 8
Время выполнения: 0.1228 секунд
Стоимость тура: 11
Длина тура: 9 вершин
Ребра веса 1: 5 (62.5%)
Ребра веса 2: 3
Оптимальная стоимость: 11
Коэффициент приближения: 1.0000
Тур: [0, 1, 7, 6, 2, 3, 5, 4, 0]

Размер графа: 8
Время выполнения: 0.1230 секунд
Стоимость тура: 11
Длина тура: 9 вершин
Ребра веса 1: 5 (62.5%)
Ребра веса 2: 3
Оптимальная стоимость: 9
Коэффициент приближения: 1.2222
Тур: [0, 2, 1, 3, 6, 4, 5, 7, 0]

Размер графа: 12
Время выполнения: 5.3790 секунд
Стоимость тура: 12
Длина тура: 13 вершин
Ребра веса 1: 12 (100.0%)
Ребра веса 2: 0
Тур: [0, 9, 6, 3, 5, 2, 11, 8, 10, 7]...

Размер графа: 15
Время выполнения: 43.5385 секунд
Стоимость тура: 16
Длина тура: 16 вершин
Ребра веса 1: 14 (93.3%)
Ребра веса 2: 1
Тур: [0, 1, 2, 11, 13, 4, 6, 3, 7, 9]...

Среднее время выполнения: 5.695 секунд
Средняя доля ребер веса 1: 80.7%
Средний коэффициент приближения: 1.0444
Максимальный коэффициент приближения: 1.2222
Теоретическая граница: 1.1429
\end{verbatim}
